...
このような学習方針は，予測精度の観点では合理的である一方で，必ずしも最終的な投資成績の改善に直結しないことが指摘されている \cite{elmachtoubgrigas2022,mandi2024}．
...
このような現象は，推定不確実性が大きい局面や，非負制約などの現実的な制約を含む設定において，特に顕在化しやすいことが知られている \cite{chopraziemba1993}．
...
$\boldsymbol{x}_i$ を用い，当該期間に実現したリターンを
$\boldsymbol{r}_i$ とする．
...
最適値一致制約 \eqref{eq:dfl_dual_value}
...
:= \inf_{\boldsymbol{w}_i\in\mathbb{R}^d}
\mathcal{L}_i(\boldsymbol{w}_i,\mu_i,\boldsymbol{\lambda}_i)
\]
と定義する．
...
\par
$\mathcal{L}_i$ を $\boldsymbol{w}_i$ で最小化する停留条件は
...
DFL-OPT-K の可行解 $(\boldsymbol{w},\mu,\boldsymbol{\lambda})$ は，
\eqref{eq:dfl_kkt_stationarity}（停留条件）と
\eqref{eq:dfl_kkt_budget}（予算制約）を満たす．
...
\]
を得る．
さらに DFL-OPT-K では相補性 \eqref{eq:dfl_kkt_complementarity} より
...
\]
が従う．
これは DFL-OPT-D の最適値一致条件 \eqref{eq:dfl_dual_value} に一致する．
...
DFL-OPT-D は
\eqref{eq:dfl_dual_stationarity}（停留条件）と
\eqref{eq:dfl_dual_budget}（予算制約）を満たすため，上と同様に
\[
\delta \boldsymbol{w}^\top \boldsymbol{V}\boldsymbol{w}
-(1-\delta)\hat{\boldsymbol{r}}^\top\boldsymbol{w}
-\mu
...
\subsection*{C.3 ヤコビアンの導出（式 \eqref{eq:jacobian_affine}）}
\par
...
\subsection*{C.4 局所Lipschitz上界（式 \eqref{eq:lipschitz_bound}）}
\par
