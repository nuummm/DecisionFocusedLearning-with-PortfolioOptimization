...
このとき多くの場合，予測モデルは平均二乗誤差などの予測誤差を最小化する目的で学習される．
このような学習方針は，予測誤差の最小化を目的とする限りにおいては妥当である一方で，必ずしも最終的な投資成績の改善に直結しないことが指摘されている \cite{butlerkwon2023}．
...
このような現象は，推定不確実性が大きい局面において特に顕著となることが古くから指摘されている \cite{chopraziemba1993}．
また，非負制約などの現実的な制約を含む設定では，推定誤差が最適な資産配分に与える影響が増幅されることが知られている．
一方で，近年では予測精度そのものではなく，後続の最適化問題を通じて得られる意思決定の質を表現した目的関数を学習目標として
直接最適化する枠組みである意思決定重視学習（Decision-Focused Learning; DFL）が注目されている \cite{mandi2024}．
\par
以上の背景から，DFL は予測値と実現値の誤差を最小化する
従来の枠組みとは異なり，
後続の最適化問題に基づいて定義される意思決定の質を学習目標とする枠組みである．
そのため，DFL をポートフォリオ最適化に応用することで，推定と最適化のギャップを緩和し，最終的な投資成績の改善につながることが期待されている．
...
を起点として発展してきた．
MVO は期待リターンとリスクを明示的に扱えることから，
理論的枠組みとしての明確さと実務上の解釈容易性を併せ持つ基準モデルとして
広く用いられている
また，MVO の入力となる期待リターンの推定は，
回帰・時系列モデルから機械学習まで幅広く研究されている一方で，
推定誤差がポートフォリオ配分に与える影響が大きいことが知られている\cite{chopraziemba1993}．

従来の多くの研究および実務では，
予測モデルを平均二乗誤差などの予測誤差を最小化する目的で学習し，
得られた推定値を最適化問題に入力して配分を決定する
二段階構造が広く採用されてきた\cite{lahoud2025}．
しかしこの枠組みでは，
予測モデルの学習目標と，
後続の最適化で最終的に評価される投資目的が
必ずしも一致しないため，
予測精度の向上が投資成績の改善に結びつかない場合が生じ得る\cite{mandi2024}．
...
DFL では，
学習過程に最適化問題を組み込み，
最適化解を介して定義される意思決定の質を
直接最小化することにより，
予測と最適化のギャップを緩和することを目指す．
近年では，
MVOに DFL を適用し，
...
Portfolio Optimization (IPO) \cite{butlerkwon2023}が提案されている．
...
本研究は，制約付き平均--分散ポートフォリオ最適化を下位問題に含む
意思決定重視学習を対象として，定式化と実証の両面から検討を行う．
特に，非負制約および予算制約を含む実務的な設定において，
予測と最適化の結合がもたらす効果と，数値計算上の取り扱いを明示的に整理することを目的とする．
本研究の主な貢献は以下のとおりである．

\begin{enumerate}
  \item 下位問題の最適性条件に基づき，意思決定重視学習の二段階最適化問題を単一レベル最適化として表現する
  2通りの再定式化（KKT 条件に基づく定式化と，強双対性に基づく最適値一致条件による定式化）を導出した．

  \item 週次TAAを想定した実データ実験を通じて，従来の予測精度重視学習や比較手法と比較して，意思決定品質の観点からの改善可能性を検証した．
  あわせて，初期化および L2 正則化による探索経路制御が，収束挙動および投資性能に与える影響を整理した．

  \item 提案手法が示した性能差の要因を理解するため，
  得られた解の構造に基づく分析を行い，挙動の解釈可能性を整理した．
  特に，共分散行列の条件数に基づくレジーム別分析や，
  極端集中確率，ポートフォリオ集中度などの指標を用いた構造分析を通じて，
  数値的に不安定な局面における意思決定誤差の挙動や配分の極端化との関係を記述した．
\end{enumerate}
...
本論文の構成は以下のとおりである．
第2章では，既存手法としてポートフォリオ最適化問題および予測統合型手法を整理する．
...
二段階構造を，
本研究では予測精度重視学習（Prediction--Focused Learning; PFL）と呼ぶ\cite{mandi2024}．
...
一方で，IPO では，非負制約や予算制約を含む制約付き MVO を扱う場合には，下位問題の最適解に依存して定義される上位目的を
パラメータ $\boldsymbol{\theta}$ について最小化する必要があるため，
$\boldsymbol{\theta}$ に関する学習問題は一般に非凸となる．
そのため，学習は最適解写像を介した勾配情報に基づく反復最適化として実行され，
計算コストの増大や初期値に依存した局所解への収束といった課題が生じ得る．
また，反復的な学習過程に依存するため，
推定モデルのパラメータと最終的な配分・性能との関係を解析的に把握しにくく，
挙動の解釈可能性が低下し得る．
...
予測モデルを学習する点に特徴がある．
この意味で，
SPO+ は最適化問題を学習ループに直接組み込む
意思決定重視学習とは異なり，
予測と最適化を分離した二段階構造に基づく手法である．
一方で，
純粋な予測精度重視学習と比較すると，
最適化問題に由来する情報を学習に反映しており，
意思決定を考慮したアプローチと位置づけられる．

本研究では，
予測精度重視学習と意思決定重視学習の中間的な位置づけを持つ手法として，
SPO+ を提案手法の比較対象として用いる．

%========================
% 悲観的二段階最適化に基づく DFL
%========================
\section{悲観的二段階最適化に基づく意思決定重視学習}
...
予測値に基づいて解かれる下位問題が
複数の最適解を持つ場合に，
その中から上位問題の目的関数を最も悪化させる解が選択されるという
悲観的解概念に基づいて後悔が定義される．
...
本節では，
制約付き平均--分散ポートフォリオ最適化（MVO）を下位問題に含む
...
期待リターンの推定値 $\hat{\boldsymbol{r}}_i(\boldsymbol{\theta},\boldsymbol{x}_i)$ は，
第2章で導入した線形モデル
...
推定値 $\hat{\boldsymbol{r}}_i(\boldsymbol{\theta},\boldsymbol{x}_i)$ に基づいて得られる
投資配分 $\hat{\boldsymbol{w}}_i(\boldsymbol{\theta},\boldsymbol{x}_i)$ は，
真のリターン $\boldsymbol{r}_i$ が既知である場合に得られる
理想的な投資配分 $\boldsymbol{w}_i^*$ と一般には一致しない．
このとき，推定に基づく意思決定が
理想的な意思決定と比較して
どの程度パフォーマンスを損なっているかを評価することが
学習目標として定める．

本研究では，この差異を
真のリターン $\boldsymbol{r}_i$ の下で評価した
目的関数値の差として定量化する．
すなわち，各時点 $i$ における損失を
\begin{equation}
...
理想的な投資配分と比較して被る
目的関数値の差分を表している．
本研究では，この損失 $\ell_i(\boldsymbol{\theta})$ を
\textbf{意思決定誤差}と呼ぶ．
...
ここで 一階の最適性条件 \eqref{eq:kkt_stationarity} と予算制約 $\boldsymbol{1}^\top\boldsymbol{w}_i=1$ を用いると，
\[
-(1-\delta)\boldsymbol{r}_i^\top \boldsymbol{w}_i
+ \frac{\delta}{2}\boldsymbol{w}_i^\top \boldsymbol{V}_i \boldsymbol{w}_i
=
...
\boldsymbol{w}_i
\]
が導出でき，整理すると以下の式を導ける．（導出の詳細は Appendix に示す．）
\begin{equation}
...
本研究では以降の解析の簡潔さの観点から，
KKT 条件に基づく定式化 DFL-OPT-K 数値実験に用いる．
...
を考える．
このとき，下位問題は目的関数が強凸であるため，最適解 $\boldsymbol{w}^*$ は一意に定まる．
さらに，不等式制約のアクティブ集合が
$\hat{\boldsymbol{r}}$ の摂動に対して局所的に不変である領域においては，
最適解 $\boldsymbol{w}^*$ は，
共分散行列の逆行列 $\boldsymbol{V}^{-1}$ による線形写像
$(1-\delta)\boldsymbol{V}^{-1}\hat{\boldsymbol{r}}$ を基点とし，
等式制約 $\boldsymbol{1}^\top\boldsymbol{w}=1$ および
アクティブ制約によって定まる可行部分空間への
射影として表される．
このとき，十分小さな推定誤差 $\Delta\hat{\boldsymbol{r}}$ に対して最適解の変動は，
\begin{equation}
...
ここで $C$ は制約集合の幾何構造およびアクティブ集合の構造にのみ依存し，
$\boldsymbol{V}$ および $\hat{\boldsymbol{r}}$ には依存しない．

したがって，制約付きMVOにおける最適解の推定誤差による増幅度は，
リスク回避パラメータ $\delta$ と
...
制約付き MVO における最適解の構造と，
推定誤差に対する局所的な安定性を
解析的に明示した点に意義がある．
特に，最適解の安定性が
共分散行列の最小固有値によって
定量的に支配されることが明らかになる．
以上より，制約付き MVO における最適解の
推定誤差に対する感度は，
リスク回避パラメータ $\delta$ と
共分散行列の最小固有値 $\lambda_{\min}(\boldsymbol V)$ によって
構造的に規定されることが分かる．
この結果は，$\delta$ が小さい場合や
市場環境および共分散推定の条件によっては，
予測誤差が大きく増幅され，
ポートフォリオ配分が不安定になり得ることを
理論的に説明している．
...
危機局面を含む長期期間における TAA の挙動を評価する目的に適している．
...
\begin{itemize}
  \item \textbf{PFL}：
  期待リターンを最小二乗誤差により推定し，
  得られた推定値 $\hat{\boldsymbol{r}}$ を MVO に入力して配分を決定する．
  実務および既存研究で広く用いられている，
...
\noindent\textbf{Decision-Focused Learning 系の参照手法}
  \begin{itemize}
	  ...
	  本研究ではパラメータ $\theta$ の初期解としても用いる．

  \item \textbf{SPO+}：
  線形最適化問題に対して
  意思決定誤差の凸上界を用いることで学習を可能にする手法
  \cite{elmachtoubgrigas2022}．
  本研究の制約付き MVO には直接適用できないため，
  予測モデルの学習に SPO+ 損失を用い，
  評価段階では他手法と同一の MVO を解く
  近似的設定により比較対象に含める．
\end{itemize}
...
また，表\ref{tab:baseline_summary}より，
...
ここでは，
解析解DFL-CFの解を $\boldsymbol{\theta}_{\mathrm{CF}}$ として $\boldsymbol{\theta}_{\mathrm{CF}}$を用いた
初期化とパラメータに対する正則化項の導入という
二つの探索経路制御手法に着目する．
...
\textbf{正則化強度の選択}
\par
パラメータ $\boldsymbol{\theta}$ が解析解 $\boldsymbol{\theta}_{\mathrm{CF}}$ から
過度に乖離することを防ぐために
目的関数には，
解析解 $\boldsymbol{\theta}_{\mathrm{CF}}$ からの過度な乖離を抑制する
L2 正則化項 $\eta \lVert \boldsymbol{\theta} - \boldsymbol{\theta}_{\mathrm{CF}} \rVert_2^2$ を導入する．
以下では，$\eta$ を正則化強度と呼ぶ．
正則化強度 $\eta$ は，
全データ期間のうち、
前半 10 年を学習期間、
...
以下の三つの設定を比較する．
\begin{itemize}
  \item 初期化なし（$\boldsymbol{\theta}=\boldsymbol{0}$）
  \item 解析解に基づく初期化（$\boldsymbol{\theta}=\boldsymbol{\theta}_{\mathrm{CF}}$）
  \item 解析解初期化＋正則化項（$\boldsymbol{\theta}=\boldsymbol{\theta}_{\mathrm{CF}}$、$\eta=5$）
\end{itemize}
...
制約付き MVO において，
下位問題における最適投資配分 $\boldsymbol{w}$ の感度が
...
\par
ここで，共分散行列 $\boldsymbol{V}_i$ の条件数は，
\begin{equation}
\kappa(\boldsymbol{V}_i)
:= \frac{\lambda_{\max}(\boldsymbol{V}_i)}{\lambda_{\min}(\boldsymbol{V}_i)}
\end{equation}
として定義する．
...
ならびに極端集中確率 $P_{\text{conc}}$ を比較した結果を示している．なお，p95 に付した括弧内の値は，
各条件数レジーム内における
意思決定誤差の平均値に対する倍率
（p95 / 平均）を表している．
ここで
...
\sum_{t=1}^T
\mathbf{1}\!\left\{
\max_{i} w_{i,t} \ge 0.95
\right\}
\]
は，
全期間において，
いずれかの時点で単一資産に 95\% 以上の投資比率が割り当てられた
頻度を表す．
...
これらの挙動は，
補題~\ref{lem:structure_stability} で示したように，
制約付き凸二次計画問題の最適解が，
アクティブ制約集合に応じて
区分的にアフィンな構造を持つという
理論的性質と整合的である．
特に，補題で示されたように，
各アクティブ集合内での解の感度が
共分散行列の最小固有値に反比例して増大するため，
条件数が高い局面では，
わずかな推定誤差であっても
アクティブ集合の切り替えが誘発されやすく，
最適解が極端なレジームへ遷移しやすい．
一方，提案手法 DFL-OPT-K では，
このようなレジーム切り替えが相対的に抑制されることで，
分布の裾における意思決定誤差の増大や
極端な集中配分の発生が回避されている可能性がある．

%------------------------
% ポートフォリオ構造と分散性の比較分析
%------------------------
\subsubsection{ポートフォリオの集中度・追随性・安定性の比較分析}
\par
前節では，共分散行列の条件数に応じて，
意思決定誤差の分布の裾における挙動や解の安定性が
大きく変化することを確認した．
本節では，
このような数値的条件の違いが，
実際にどのようなポートフォリオ構造として
現れているかを分析する．

\begin{table}[H]
\centering
\caption{ポートフォリオの集中度・追随性・安定性の比較}
\label{tab:portfolio_structure_summary}
\small
\setlength{\tabcolsep}{7pt}
\begin{tabular}{lrrr}
\hline
Model & 捕捉率（\%） & HHI & Turnover \\
\hline
\rowcolor{gray!10}
DFL-OPT-K & \textbf{43.1} & \textbf{0.7720} & 24.01 \\
DFL-CF    & 27.2 & 0.9676 & 24.17 \\
IPO-GRAD  & 27.3 & 0.9669 & 25.24 \\
SPO+      & 30.1 & 0.9695 & 21.02 \\
PFL       & 30.6 & 0.9468 & \textbf{20.73} \\
\hline
\end{tabular}
\end{table}
...
各手法により生成される
ポートフォリオ配分 $\{\boldsymbol{w}_t\}_{t=1}^T$ の
時系列的な構造特性を要約したものである．
ここでは，
集中度を表す Herfindahl--Hirschman Index（HHI），
週次リターン最大資産を十分なウェイトで捕捉している割合を捕捉率，
およびポートフォリオの更新頻度を表す平均ターンオーバーに着目する．

\par
HHI は各時点のポートフォリオ配分に対して
$\mathrm{HHI}_t := \sum_{i=1}^d w_{i,t}^2$ と定義し，
その時系列平均を報告している．
また，平均ターンオーバー（ Turnover ）は，
隣接時点間のポートフォリオ変更量
$\|\boldsymbol{w}_t-\boldsymbol{w}_{t-1}\|_1/2$
の時系列平均として定義する．

\par
捕捉率は，
各時点 $t$ において実現リターンが最大となった資産
\[
j_t := \arg\max_j r_{j,t}
\]
に対して，
予測に基づく投資配分 $\boldsymbol{w}_t$ が
一定以上のウェイトを割り当てていたかどうかを判定し，
その頻度を集計した指標である．
本研究では代表的なしきい値として $0.05$ を用い，
捕捉率を次式で定義する．
\[
\text{捕捉率}
:=
...
\par
本節では，
市場の急激な変動が生じた危機局面における
各手法の挙動を比較・検証する．
表\ref{tab:crisis_event_metrics}には，
各危機局面における
Sharpe 比および下方リスク指標 CVaR$_{95}$ を示す．

\begin{table}[H]
\centering
\caption[危機局面における性能指標の比較]{危機局面\protect\footnotemark における性能指標の比較}
...
\footnotetext{
各危機局面の評価期間は，
リーマンショック期（2007/07/01--2009/06/30），
COVID-19 初期局面（2020/02/01--2020/12/31），
インフレ局面（2022/01/01--2023/12/31）
として設定した．
}

\par
表\ref{tab:crisis_event_metrics}より，
多くの比較手法では，
危機局面において Sharpe 比が大きく低下すると同時に，
CVaR$_{95}$ が顕著に増大していることが確認される．
これは，
急激な市場変動下では，
推定誤差が意思決定に増幅されやすく，
下方リスクが顕在化しやすいことを示唆している．
一方，
提案手法 DFL-OPT-K は，
すべての危機局面において
CVaR$_{95}$ を相対的に低水準に抑制しており，
特に COVID-19 初期局面およびインフレ局面では，
他手法と比較して高い Sharpe 比を維持している．
この結果は，
市場急変時においても，
提案手法が極端な損失を回避しつつ，
比較的安定した意思決定を継続していることを示唆する．
これらの挙動は，
前節までで確認した
高条件数レジームにおける
意思決定誤差の分布の裾における抑制や
...
本節では，
資産ユニバースを拡張した場合においても，
提案手法 DFL-OPT-K により確認された性能傾向が
維持されるかを検証する．
これまでの実験では，
米国株式（SPY）・新興国株式（EEM）・債券（TLT）・金（GLD）からなる，
基本的な分散構造を持つ
4 資産設定を基準ケースとして用いてきた．
本節では，
この基準設定に対して，
地域分散，株式スタイル分散，
および金利感応度の違いを段階的に追加することで，
6 資産および 8 資産設定を構成する．
いずれの設定においても，
学習・評価の枠組みおよびハイパーパラメータは
4 資産設定と同一とする．

\par
6 資産設定では，
4 資産設定に対して，
先進国株式（EFA）および中期国債（IEF）を追加する．
具体的な資産構成は次の通りである.

\[
\text{SPY, EFA, EEM, IEF, TLT, GLD}.
\]
...
具体的な資産構成は次の通りである.
\[
\text{SPY, QQQ, IWM, EFA, EEM, IEF, TLT, GLD}.
\]

ここで，
QQQ は米国大型グロース株，
IWM は米国小型株へのエクスポージャを提供する．
これにより，
株式部分において
市場全体・成長株・小型株という
異なるリスク特性が導入される．
...
\par
図\ref{fig:wealth_events_6assets}および図\ref{fig:wealth_events_8assets}より，
6 資産・8 資産のいずれの設定においても，
DFL-OPT-K は長期運用期間を通じて
比較的滑らかな資産成長を示していることが確認できる．
特に，
危機局面を含む期間においても，
資産価値の急激な毀損が抑制されている点は，
4 資産設定と同様である．
また，
表\ref{tab:summary_6assets}および表\ref{tab:summary_8assets}に示す通り，
DFL-OPT-K は，
Sharpe 比および最終資産価値において
学習ベースの比較手法を一貫して上回っている．
同時に，
CVaR$_{95}$ および年率ボラティリティは
等分散投資（$1/N$）に次ぐ水準に留まっており，
高いリターン性能と下方リスク抑制のバランスが，
資産数の増加後も維持されていることが確認できる．
以上の結果から，
資産ユニバースを 6 資産および 8 資産へと拡張した場合においても，
提案手法 DFL-OPT-K は，
4 資産設定で確認された性能傾向を概ね維持していることが確認された．
...
意思決定重視学習に着目し，
定式化と実データ検証の両面から検討を行った．
従来の予測精度重視学習では，
予測誤差が小さい場合であっても，
最適化段階で誤差が増幅され，
配分が不安定化し得るという課題がある．
本研究ではこの点を踏まえ，
下位問題の最適性条件に基づく単一レベル再定式化を与え，
意思決定品質を直接最小化する学習枠組みを整理した．
また、週次 TAA を想定した実データ実験を通じて，
提案手法は予測精度が必ずしも高くないにもかかわらず，
Sharpe 比および下方リスク指標の観点で
良好な投資成績を示す傾向が確認された．
また，
共分散行列の条件数に基づくレジーム別分析では，
高条件数局面において多くの手法で意思決定誤差の分布の裾が拡大する一方，
極端集中確率および誤差分布の裾部における増幅が，
相対的に抑制される傾向が観察された．
さらに，
ポートフォリオ構造の観点からは，分散性を保ちつつ捕捉率も高いという形で，
極端な集中に依存しない意思決定を行っていることが示された．
危機局面においても，
下方リスクを抑制しつつ比較的安定した挙動を示す傾向が確認された．
加えて，
資産ユニバースを 6 資産・8 資産へ拡張した場合でも，
本研究の範囲において，
学習ベースの比較手法に対して同様の性能傾向が概ね維持されることが確認された．

\par
なお，本研究の実証結果は，
本研究で設定した資産ユニバースおよび運用条件の下で得られたものである．
また，
提案手法は非凸最適化問題として定式化されており，
本研究では汎用的な非線形最適化ソルバーを用いて解いているため，
計算時間はソルバー設計や実装に強く依存する．
今後は，
問題構造を活かした近似解法やヒューリスティックなアルゴリズムの設計により，
より高頻度な意思決定や大規模設定への拡張可能性を検討する余地がある．
...
本研究にあたり，終始丁寧なご指導と貴重なご助言を賜りました高野祐一先生に，心より感謝申し上げます．
また，中間発表をご担当いただいた黒瀬雄大先生をはじめ，本論文の執筆に際して多くのご助言をくださった皆様にも，厚く御礼申し上げます．
...
\bibitem{bucarey2024}
V.~Bucarey, S.~Calder{\'o}n, G.~Mu{\~n}oz, and F.~Semet.
...
\newblock In {\em International Conference on the Integration of Constraint
  Programming, Artificial Intelligence, and Operations Research (CPAIOR 2024)},
  Lecture Notes in Computer Science. Springer, 2024.
\newblock DOI: 10.1007/978-3-031-60597-0\_9.

\bibitem{butlerkwon2023}
A.~Butler and R.~H. Kwon.
\newblock Integrating prediction in mean--variance portfolio optimization.
\newblock {\em Quantitative Finance}, 23(3):429--452, 2023.
\newblock DOI: 10.1080/14697688.2022.2092829.

\bibitem{chen2010oas}
Y.~Chen, A.~Wiesel, and A.~O. Hero.
\newblock Shrinkage estimation of high-dimensional covariance matrices.
\newblock {\em IEEE Transactions on Signal Processing}, 58(10):5016--5029,
  2010.
\newblock DOI: 10.1109/TSP.2010.2053029.

\bibitem{chopraziemba1993}
V.~K. Chopra and W.~T. Ziemba.
\newblock The effect of errors in means, variances, and covariances on optimal
  portfolio choice.
\newblock {\em The Journal of Portfolio Management}, 19(2):6--11, 1993.
\newblock DOI: 10.3905/jpm.1993.409440.
...
J.~P. Morgan.
\newblock {\em RiskMetrics\texttrademark\ Technical Document}, 4th edition,
  J.P. Morgan, 2006.

\bibitem{lahoud2025}
A.~A. Lahoud, A.~S. Khan, E.~Schaffernicht, M.~Trincavelli, and J.~A. Stork.
\newblock Predict-and-optimize techniques for data-driven optimization
  problems: A review.
\newblock {\em Neural Processing Letters}, 57(2): Article 40, 2025.
\newblock DOI: 10.1007/s11063-025-11746-w.

\bibitem{lee2025}
J.~Lee, H.~Jeon, H.~Bae, and Y.~Lee.
\newblock Return prediction for mean--variance portfolio selection: How
  decision-focused learning shapes forecasting models.
\newblock In {\em Proceedings of the 6th ACM International Conference on AI in
  Finance (ICAIF '25)}, pages 114--122, 2025.

\bibitem{mandi2024}
J.~Mandi, J.~Kotary, S.~Berden, M.~Mulamba, V.~Bucarey, and F.~Fioretto.
\newblock Decision-focused learning: Foundations, state of the art, benchmark
  and future opportunities.
\newblock {\em Journal of Artificial Intelligence Research}, 81:1623--1701,
  2024.
\newblock DOI: 10.1007/s11063-025-11746-w.  % if DOI differs, check publisher page
...
\bibitem{shah2022}
S.~Shah, K.~Wang, H.~Chen, A.~Perrault, F.~Doshi-Velez, and M.~Tambe.
\newblock Decision-focused learning without decision-oracles: Learning locally
  optimized decision losses.
\newblock In {\em Advances in Neural Information Processing Systems 35 (NeurIPS
  2022)}, 2022.
...
%========================
\newpage
\refstepcounter{chapter} % Appendix を第6章として扱う（図表番号を 6.x にする）
\chapter*{Appendix}
...
% Appendix A
%========================
\section*{A. DFL-OPT-D における強双対性制約の導出}
\addcontentsline{toc}{section}{A. DFL-OPT-D における強双対性制約の導出}
\par
本節では，本文で導入した DFL-OPT-D における
主問題と双対問題の目的関数の一致から導かれる \eqref{eq:dfl_dual_value}
（同値な形として \eqref{eq:dual_strong_simplified}）の導出を示す．
導出は，下位問題の双対関数と強双対性に基づく．

\par
各時点 $i$ における下位問題 \eqref{eq:lower_level_recall} に対し，
ラグランジュ関数 \eqref{eq:lagrangian} を用いて，
双対関数を
\begin{equation}
g_i(\mu_i,\boldsymbol{\lambda}_i)
:= \inf_{\boldsymbol{w}_i}
\mathcal{L}_i(\boldsymbol{w}_i,\mu_i,\boldsymbol{\lambda}_i)
\end{equation}
と定義する．
ここで $\boldsymbol{\lambda}_i\ge\boldsymbol{0}$ とする．

\par
$\mathcal{L}_i$ を $\boldsymbol{w}_i$ で最小化する一階の最適性条件は
\eqref{eq:dfl_dual_stationarity} と一致し，
これより
\begin{equation}
\tilde{\boldsymbol{w}}_i(\mu_i,\boldsymbol{\lambda}_i)
=
\frac{1}{\delta}\boldsymbol{V}_i^{-1}
\!\left(
(1-\delta)\hat{\boldsymbol{r}}_i
+\mu_i\boldsymbol{1}
+\boldsymbol{\lambda}_i
...
\par
次に，一階の最適性条件 \eqref{eq:dfl_dual_stationarity} 
\begin{equation}
\delta \boldsymbol{V}_i\boldsymbol{w}_i
=
(1-\delta)\hat{\boldsymbol{r}}_i
...
\par
弱双対性より，任意の主問題の実行可能解 $\boldsymbol{w}_i$ と
双対変数 $(\mu_i,\boldsymbol{\lambda}_i)$ に対して，
主問題の目的値は双対関数値以上である．
\begin{equation}
-(1-\delta)\hat{\boldsymbol{r}}_i^\top\boldsymbol{w}_i
+\frac{\delta}{2}\boldsymbol{w}_i^\top\boldsymbol{V}_i\boldsymbol{w}_i
...
\par
下位問題は Slater 条件を満たす凸最適化問題であるため，
強双対性が成立し，
最適解において上記の不等式は等号で成り立つ．
このとき KKT 条件の相補性により
$\boldsymbol{\lambda}_i^\top\boldsymbol{w}_i=0$ が成り立ち，
双対変数は最適値表現から消去される．
したがって，
最適解 $(\boldsymbol{w}_i,\mu_i)$ は
\begin{equation}
\delta \boldsymbol{w}_i^\top\boldsymbol{V}_i\boldsymbol{w}_i
-
(1-\delta)\hat{\boldsymbol{r}}_i^\top\boldsymbol{w}_i
...
を満たす．
これは本文で用いた
\eqref{eq:dfl_dual_value}
（同値に \eqref{eq:dual_strong_simplified}）である．

%========================
% Appendix B
%========================
\section*{B. 補題 \ref{prop:equivalence} の証明}
\addcontentsline{toc}{section}{B. 補題 \ref{prop:equivalence} の証明}
\par
本節では，DFL-OPT-D と DFL-OPT-K が同一の解集合を持つことを示す．
\begin{proof}
各 $i=1,\dots,T$ に対して，下位問題 \eqref{eq:lower_level_recall} は
$\delta>0$ および $\boldsymbol{V}_i\succ\boldsymbol{0}$ の下で強凸な目的関数と凸制約集合を持つ凸二次計画問題である．
また，仮定より Slater 条件が成立するため強双対性が成り立ち，
KKT 条件は最適性の必要十分条件である．

\par
以下では，各時点 $i$ を固定して示す（添字 $i$ は省略する）．

\par\noindent
\textbf{(DFL-OPT-K $\Rightarrow$ DFL-OPT-D)}\;
DFL-OPT-K の可行解 $(\boldsymbol{w},\mu,\boldsymbol{\lambda})$ は，一階の最適性条件
\eqref{eq:dfl_kkt_stationarity}と予算制約
\eqref{eq:dfl_kkt_budget}を満たす．
したがって，\eqref{eq:dfl_kkt_stationarity} の両辺に $\boldsymbol{w}^\top$ を掛け，
$\boldsymbol{1}^\top\boldsymbol{w}=1$ を用いると
\begin{equation}
\delta \boldsymbol{w}^\top \boldsymbol{V}\boldsymbol{w}
...
\end{equation}
を得る．
さらに DFL-OPT-K では相補性条件 \eqref{eq:dfl_kkt_complementarity} より
$\boldsymbol{\lambda}^\top\boldsymbol{w}=0$ が成り立つため，
\begin{equation}
\delta \boldsymbol{w}^\top \boldsymbol{V}\boldsymbol{w}
-(1-\delta)\hat{\boldsymbol{r}}^\top\boldsymbol{w}
=\mu
\end{equation}
が従う．
これは DFL-OPT-D の \eqref{eq:dfl_dual_value} に一致する．
他の制約（主・双対可行性および停留条件）は両定式化で共通であるから，
DFL-OPT-K の可行解は DFL-OPT-D の可行解でもある．

\par\noindent
\textbf{(DFL-OPT-D $\Rightarrow$ DFL-OPT-K)}\;
逆に DFL-OPT-D の可行解 $(\boldsymbol{w},\mu,\boldsymbol{\lambda})$ をとる．
DFL-OPT-D は一階の最適性条件
\eqref{eq:dfl_dual_stationarity}と予算制約
\eqref{eq:dfl_dual_budget}を満たすため，上と同様に
\begin{equation}
\delta \boldsymbol{w}^\top \boldsymbol{V}\boldsymbol{w}
-(1-\delta)\hat{\boldsymbol{r}}^\top\boldsymbol{w}
-\mu
...
\end{equation}
が成り立つ．
一方，DFL-OPT-D は最適値一致 \eqref{eq:dfl_dual_value} を課しているので，
左辺は $0$ となり $\boldsymbol{\lambda}^\top\boldsymbol{w}=0$ を得る．
ここで $\boldsymbol{w}\ge\boldsymbol{0}$ および $\boldsymbol{\lambda}\ge\boldsymbol{0}$ より，
$\lambda_j w_j\ge 0$ が全成分で成り立つから，
和が $0$ であることは各成分で $\lambda_j w_j=0$ を意味する．
したがって
\begin{equation}
\boldsymbol{\lambda}\odot\boldsymbol{w}=\boldsymbol{0}
\end{equation}
が従い，DFL-OPT-K の相補性条件 \eqref{eq:dfl_kkt_complementarity} が得られる．
ゆえに DFL-OPT-D の可行解は DFL-OPT-K の可行解でもある．

\par
以上より，両定式化は同一の可行集合を持つ．目的関数 \eqref{eq:dfl_kkt_obj}, \eqref{eq:dfl_dual_obj} は共通であるため，
DFL-OPT-D と DFL-OPT-K は同一の解集合を持つ．
\end{proof}
...
% Appendix C: 補題 の導出
%========================
\section*{C. 補題 \ref{lem:structure_stability} の導出}
\addcontentsline{toc}{section}{C. 補題3.2 の導出}
\par
本節では，補題 \ref{lem:structure_stability} の導出を示す．
以下では簡単のため時点添字を省略し，
\eqref{eq:lower_level_recall} の下位問題
\begin{equation}
\min_{\boldsymbol{w}}
...
このとき，制約 $\boldsymbol{w}\ge\boldsymbol{0}$ は局所的に
\begin{equation}
...
\label{eq:wA_affine_raw}
\end{equation}

\par
式 \eqref{eq:wA_affine_raw} は整理すると，
\begin{equation}
\boldsymbol{w}_A^*
=
\frac{1-\delta}{\delta}
...
\boldsymbol{1}_A^\top
\Bigr)\boldsymbol{V}_{AA}^{-1}\hat{\boldsymbol{r}}_A
+
\frac{\boldsymbol{V}_{AA}^{-1}\boldsymbol{1}_A}
{\boldsymbol{1}_A^\top \boldsymbol{V}_{AA}^{-1}\boldsymbol{1}_A}
\label{eq:wA_affine_explicit}
\end{equation}
と書き直すことができる．
したがって，同一のアクティブ集合 $A$ が維持される局所では，
$\boldsymbol{w}_A^*$ は $\hat{\boldsymbol{r}}_A$ に関して
線形項と定数項からなるアフィン写像である．
...
\end{equation}
が成り立つ．したがって 2-ノルムの誘導ノルムの性質より，
\[
\|\Delta \boldsymbol{w}_A^*\|_2
=
\left\|\left(\frac{\partial \boldsymbol{w}_A^*}{\partial \hat{\boldsymbol{r}}_A}\right)\Delta\hat{\boldsymbol{r}}_A\right\|_2
\le
\left\|
\frac{\partial \boldsymbol{w}_A^*}{\partial \hat{\boldsymbol{r}}_A}
\right\|_2
...
とおく（ $P_A\in\mathbb{R}^{|A|\times|A|}$ ）と，
\begin{equation}
\frac{\partial \boldsymbol{w}_A^*}{\partial \hat{\boldsymbol{r}}_A}
=
\frac{1-\delta}{\delta}
...
\end{equation}
（$V_{AA}\succ0$ より対称正定値であり，$\|M\|_2=\lambda_{\max}(M)$ を用いると
$\|V_{AA}^{-1}\|_2=\lambda_{\max}(V_{AA}^{-1})=1/\lambda_{\min}(V_{AA})$）
これより
\begin{equation}
\|\Delta \boldsymbol{w}_A^*\|_2
\le
...
\par
以上で補題 \ref{lem:structure_stability} の導出を完了する．
...
% Appendix D
%========================
\section*{D. OAS 縮小係数と EWMA の実効サンプルサイズ}
\addcontentsline{toc}{section}{D. OAS 縮小係数と EWMA の実効サンプルサイズ}
\par
本節では，第4章で用いたEWMA 共分散に対する OAS 縮小の実装上の対応関係を明記する．
OAS の縮小係数はサンプルサイズ $n$ に依存するため，本研究では EWMA の重み付けにより得られる
実効サンプルサイズ $n_{\mathrm{eff}}$ を $n$ の代替として用いる．

\par
まず，$d$ 次元の標本共分散を $\bm S_t$ とし，
OAS による縮小係数を $\phi_t$ とする．このとき $\phi_t$ は文献 \cite{chen2010oas} に従い
\begin{equation}
\phi_t
= \min\left\{
1,\ 
\frac{\left(1-\frac{2}{d}\right)\mathrm{tr}(\bm S_t^2)+\mathrm{tr}(\bm S_t)^2}
{\left(n_{\mathrm{eff}}+1-\frac{2}{d}\right)\left(\mathrm{tr}(\bm S_t^2)-\frac{\mathrm{tr}(\bm S_t)^2}{d}\right)}
\right\}
\label{eq:appendix_oas_phi}
\end{equation}
で与えられる．

\par
次に，EWMA の減衰率を $\alpha\in(0,1)$，有限窓長を $L$ とし，
正規化重みを $\tilde{w}_k := \frac{(1-\alpha)\alpha^k}{1-\alpha^L}$（$k=0,\dots,L-1$）と定義する．
このとき実効サンプルサイズを
\begin{equation}
n_{\mathrm{eff}}
:= \frac{1}{\sum_{k=0}^{L-1}\tilde{w}_k^2}
= \frac{1+\alpha}{1-\alpha}\cdot\frac{(1-\alpha^L)^2}{1-\alpha^{2L}}
\label{eq:appendix_neff}
\end{equation}
とおく．特に $L$ が十分大きい場合には
$n_{\mathrm{eff}} \approx \frac{1+\alpha}{1-\alpha}$ となり，
EWMA の記憶長の近似と整合する \cite{jpm2006riskmetrics}．

%========================
% Appendix E: 取引コスト控除後の解析解に基づく初期化および L2 正則化項ありの参考結果
%========================
\section*{E. 取引コスト控除後の解析解に基づく初期化および L2 正則化項ありの参考結果}
\addcontentsline{toc}{section}{E. 取引コスト控除後の解析解に基づく初期化および L2 正則化項ありの参考結果}
\par
本付録では，第4章で検討した探索経路制御（解析解に基づく初期化および L2 正則化項）について，
取引コスト控除後の実行ベース損益系列に基づく全期間の累積資産推移を参考として示す．
本結果は提案手法の優位性を主張するための追加実験ではなく，
探索経路制御が実務的なコストを考慮した場合にも性能を著しく損なわないことを確認する目的で提示する．

\par
取引コストはティッカー別に（片道）\texttt{SPY: 5 bps, GLD: 10 bps, EEM: 10 bps, TLT: 5 bps}
として設定し，各リバランス時点の売買量に比例して控除した（他の実験条件は本文と同一）．

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figs/appendix_fullperiod_wealth_penalty.png}
\caption{参考：取引コスト控除後の解析解に基づく初期化および L2 正則化項ありの累積資産推移}
\label{fig:appendix_fullperiod_wealth_penalty}
\end{figure}

\begin{table}[H]
\centering
\caption{参考：取引コスト控除後の解析解に基づく初期化および L2 正則化項ありの性能指標}
...
\hline
\rowcolor{gray!10}
DFL-OPT-K & \textbf{0.83} & \textbf{12.15} & \textbf{13.95} & \underline{16.76} & \underline{5.21} \\
DFL-CF & 0.56 & 5.07 & 9.66 & 17.36 & 5.65 \\
IPO-GRAD & \underline{0.62} & \underline{6.15} & \underline{10.62} & 17.25 & 5.51 \\
SPO+ & 0.55 & 5.15 & 9.84 & 17.92 & 5.73 \\
PFL & 0.29 & 2.11 & 5.61 & 19.04 & 6.68 \\
Buy\&Hold(SPY) & 0.57 & 5.64 & 10.35 & 18.14 & 6.19 \\
1/N & \underline{0.62} & 3.64 & 7.13 & \textbf{11.42} & \textbf{3.65} \\
\hline
\end{tabular}
\end{table}

\par
図\ref{fig:appendix_fullperiod_wealth_penalty}および表\ref{tab:appendix_fullperiod_summary_cost}より，
探索経路制御を導入した設定においても，
取引コスト控除後の長期推移が大きく崩れないことが確認できる．
