...
% Appendix A
%========================
\section*{A. 補題 \ref{prop:equivalence} の証明}
\addcontentsline{toc}{section}{A. 補題 \ref{prop:equivalence} の証明}
\par
...
% Appendix C
%========================
\section*{C. OAS 縮小係数と EWMA の実効サンプルサイズ}
\addcontentsline{toc}{section}{C. OAS 縮小係数と EWMA の実効サンプルサイズ}
\par
...
%========================
% Appendix D: 初期解＋正則化あり（全期間）の参考結果
%========================
\section*{D. 初期解＋正則化あり（全期間）の参考結果}
\addcontentsline{toc}{section}{D. 初期解＋正則化あり（全期間）の参考結果}
\par
本付録では，第4章で検討した探索経路制御（解析解に基づく初期化および L2 正則化項）について，
取引コスト控除後の実行ベース損益系列に基づく全期間（2006--2025）の累積資産推移を参考として示す．
...
\caption{全期間（2006--2025）における累積資産推移（初期解＋正則化，取引コスト控除後；参考）}
\label{fig:appendix_fullperiod_wealth_penalty}
\end{figure}

\begin{table}[H]
\centering
\caption{全期間（2006--2025）における性能指標（初期解＋正則化，取引コスト控除後；参考）}
\label{tab:appendix_fullperiod_summary_cost}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lrrrrr}
\hline
Model & Sharpe 比 & 最終資産 & 年率リターン(\%) & 年率ボラ(\%) & CVaR$_{95}$ \\
\hline
\rowcolor{gray!10}
DFL-OPT-K & \textbf{0.83} & \textbf{12.15} & \textbf{13.95} & \underline{16.76} & \underline{5.21} \\
DFL-CF & 0.56 & 5.07 & 9.66 & 17.36 & 5.65 \\
IPO-GRAD & 0.57 & 5.25 & 9.85 & 17.39 & 5.63 \\
SPO+ & 0.55 & 4.90 & 9.50 & 17.38 & 5.67 \\
PFL & 0.29 & 2.11 & 5.61 & 19.04 & 6.68 \\
Buy\&Hold(SPY) & 0.57 & \underline{5.64} & \underline{10.35} & 18.14 & 6.19 \\
