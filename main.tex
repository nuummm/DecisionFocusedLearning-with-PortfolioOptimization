...
意思決定重視学習（DFL） が提案されている\cite{mandi2024}．
DFL では，
学習過程に最適化問題を組み込み，
...
意思決定重視学習を対象として，
定式化および数値計算上の取り扱いの両面から体系的な検討を行った点に特徴がある．
特に，非負制約および予算制約を含む実務的な設定において，
予測と最適化を結合した学習問題がどのような構造を持ち，
それが数値計算および意思決定結果にどのような影響を及ぼすかを明示的に整理した．

まず，意思決定重視学習における二段階最適化問題に対し，
下位問題の最適性条件に基づく二通りの再定式化を導出し，
単一レベルの最適化問題として取り扱えることを示した．
これにより，理論的構造と数値計算上の実装との対応関係を明確化した．
次に，実データ実験を通じて，
従来の予測精度重視学習や既存の比較手法と比較し，
意思決定品質の観点からの改善可能性を検証した．
さらに，得られた解の構造に基づく分析を行うことで，
数値的に不安定な局面における意思決定誤差の挙動と
資産配分の極端化との関係を明らかにした点も本研究の貢献である．
...
% Appendix A
%========================
\section{付録 A：DFL-OPT-D における強双対性制約の導出}
\par
...
正規化重みを
$\tilde{w}_k := \frac{(1-\alpha)\alpha^k}{1-\alpha^L}$（$k=0,\dots,L-1$）
と定義する．
このとき，実効サンプルサイズを
\begin{equation}
n_{\mathrm{eff}}
...
と定める．
特に $L$ が十分大きい場合には
$n_{\mathrm{eff}} \approx \frac{1+\alpha}{1-\alpha}$ が成り立ち，
EWMA における記憶長の近似表現と一致する．
また，本研究では業界慣例に従い，$\alpha = 0.97$ を採用する
\cite{jpm2006riskmetrics}．

%========================
% Appendix E: 取引コスト控除後の解析解に基づく初期化および正則化項ありの参考結果
%========================
\section{付録 E：参考結果（初期化・正則化項あり）}
\par
本付録では，第4章で検討した探索経路制御
（解析解に基づく初期化および正則化項）について，
取引コスト控除後の実行ベース損益系列に基づく
全期間の累積資産推移を参考として提示する．
本結果は提案手法の優位性を主張するための追加実験ではなく，
探索経路制御が実務的な取引コストを考慮した場合においても，
性能低下が限定的であることを確認する目的で示すものである．

\par
取引コストは資産別に片道手数料率を
\texttt{SPY: 5 bps, GLD: 10 bps, EEM: 10 bps, TLT: 5 bps}
と設定し，各リバランス時点における売買量に比例して控除した
（その他の実験条件は本文と同一である）．

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figs/appendix_fullperiod_wealth_penalty.png}
\caption{参考：取引コスト控除後の解析解に基づく初期化および正則化項ありの累積資産推移}
\label{fig:appendix_fullperiod_wealth_penalty}
\end{figure}

\begin{table}[H]
\centering
\caption{参考：取引コスト控除後の解析解に基づく初期化および正則化項ありの性能指標}
